...
object LocationService {
    ...
    /**
     * Returns the last known location WITHOUT expiry check.
     * Used for alarm re-scheduling where a slightly stale location
     * is far better than missing all remaining prayer alarms.
     */
    fun getLastKnownLocationForScheduling(context: Context): Location? {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val lat = prefs.getFloat(KEY_LAT, Float.MAX_VALUE).toDouble()
        val lon = prefs.getFloat(KEY_LON, Float.MAX_VALUE).toDouble()
